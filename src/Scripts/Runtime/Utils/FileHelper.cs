...
namespace Racer.EzSaver.Scripts.Runtime.Utils
{
    internal static class FileHelper
    {
        private static string[] FilteredExtensions { get; } = { ".meta" };
        public static string SaveDirPath => EzSaverConfig.Instance.FileRootPath;
        ...
        public static string AssignExtension(string filename)
        {
            try
            {
                return Path.HasExtension(filename)
                    ? filename
                    : filename + EzSaverConfig.Instance.SaveFileExtension;
            }
            catch (Exception e)
            {
                Debug.LogError(e);
