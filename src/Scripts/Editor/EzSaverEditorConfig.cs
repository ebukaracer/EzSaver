...
namespace Racer.EzSaver.Scripts.Editor
{
    ...
    internal class EzSaverEditorConfig : UnityEditor.Editor
    {
        ...
        private void TryLoadBackupFile()
        {
            ...
            bool TryRestoreBackup()
            {
                ...
                try
                {
                    ...
                    var decrypted = EzSaverFileUtility.DecryptString(_loadedBackup.testCipherText);
        ...
        {
            var path = EditorUtility.SaveFilePanel("Save Credentials to File", "",
                $"{PlayerSettings.productName}_ezsaver_backup.json", "json");
            ...
            var backup = new EzSaverKeyBackup
            {
                ...
                testCipherText = EzSaverFileUtility.EncryptString(EncDecTestStr)
            };

            File.WriteAllText(path, JsonUtility.ToJson(backup, true));
            Debug.Log($"Credentials successfully saved to: {path}");
        }
